...
${visibility} class ${name} implements ${super} {
...
<#list interceptors as interceptor>
    ...
    private final ${super} $delegate;

    public ${name}(final ${super} $delegate, <#list interceptors as interceptor>final ${interceptor.type} ${interceptor.name}<#sep>, </#sep></#list>) {
...
<#list methods as method>
    public <#if method.returnType??>${method.returnType}<#else>void</#if> ${method.name}(<#list method.parameters as parameter>final ${parameter.type} ${parameter.name}<#sep>, </#sep></#list>) <#list method.exceptions as eT><#if eT?is_first>throws </#if>${eT}<#sep>, </#sep></#list> {
        <#if method.intercepted?? >
        Callable<Object> $c = new Callable<Object>() {
            public Object call() throws Exception {
                <#if method.returnType??>return </#if>$delegate.${method.name}(<#list method.parameters as parameter>${parameter.name}<#sep>, </#sep></#list>);
